...
-- Q5: What was the best selling video game worldwide? Round global sales numbers to 2 decimal places. 
...
-- Q6: List all the details for the video games that were published by Nintendo and released between 2007 and 2010
...
-- Q7: List the name, publisher, North American (NA-Sales), and the year video games were published by Take-Two Interactive in 2007.  List  North America Sales Numbers by desc order. 
...
-- Q8: List the name, global_sales and platform for the video games that have global_sales higher then 30 (million) in the sports genre.

Select name, global_sales, platform, genre from vgsales1 where Genre = 'Sports' and global_sales > 30; 



-- Q9: Count the number of video games published by Microsoft in 2008 in different genres. 
...
-- Q10: What was the best-selling video game worldwide on the SNES
...
-- Q11: What was the best selling video game published by Microsoft Game Studios in Europe in 2010?
...
-- Q12: count the number of titles published by Microsoft Game Studios in the Shooter Genre in 2010
...
-- Q13: count the number of titles released on the Nintendo 3ds in the sports genre in 2013
select genre, count(name), year
from vgsales1
where platform = '3ds' and genre = 'Sports' and year = '2013'
group by genre, year

-- Q14: List the name and NA-Sales for the video games that have NA_sales higher then 20 (million) from each genre

Select name, NA_Sales, genre from vgsales1 group by genre, NA_Sales, name having NA_Sales > 20

-- Q15: List the name, global sales and genre for the video games that have global_sales between 20 and 40 (million) from each genre 

Select name, global_Sales, genre from vgsales1 group by name, global_Sales, genre having global_sales between 20 and 40;

-- Q16: List the name, platform, publisher and year of the video games that were published by Sony Computer Entertainment or Microsoft Game Studios in 2007.
...
-- Q17: List all of the details for the video games published by Take Two Interactive and Sony Computer Entertainment in 2006. 

Select * from vgsales1 where Year = '2006' and (publisher = 'Take-Two Interactive' or publisher = 'Sony Computer Entertainment')

-- Q18: count the number of different genres in this dataset

Select count(distinct(genre)) as Number_of_Different_Genres from vgsales1

-- Q19: count the number of different platforms in this dataset

Select count(distinct(platform)) as Number_of_different_platforms from vgsales1

-- Q20: What is the average global sales for video games in the Racing Genre published by Sony Computer Entertainment in 2001
...
-- Q21: What is the average NA_Sales for video games in the Action  Genre published by Microsoft Game Studios in 2004
...
-- Q22: What are the total global sales by genre in this dataset? List Sales in Ascending Order

Select genre, SUM(global_sales) as Total_Sales from vgsales1 group by genre order by Total_Sales asc 

-- Q23: Display the top 10 publishers with NA_Sales in this dataset  List NA_Sales in Descending Order. Rank Total Sales in desc order. 
...
--Q24: Display top 5 video game EU sales by genre. List EU_Sales in descending order. Rank EU Sales in descending order. 
...
-- Q25: List all of the details for the video games in this dataset that start with the letter "W" and released on the PS4. 

Select * from vgsales1 where name like 'W%' and Platform = 'PS4';

-- Q26: List all of the details for the video games that end with the letter 'B' in this entire dataset. 

Select * from vgsales1 where name like '%B' 

-- Q27: List the name, platform of the video games that contains the word "MLB" anywhere in it name. 

Select name, platform from vgsales1 where name like '%MLB%'

-- Q28: List the name, platform, publisher, year that has the letter 'O' in its second position. 

Select name, platform, publisher, year from vgsales1 where name like '_o%'

--Q29: What was the best selling video game in North America on the PS4?
